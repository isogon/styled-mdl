...
const log = (pass) => (strings, thing, expected, actual) => {
  ...
  return `${wanted} ${thing.type().displayName}${pass ? ' not' : ''} ${toBe}:\n${chalk.green(expected)}\n\n${received}:\n${chalk.red(actual)}`;
};
...
const matchers = {
  toHaveInnerText() {
    return {
      compare(actual, expected) {
        ...
        const message = log(pass)`Expected ${actual} to have text ${expected} but it was ${actualText}`;
        return { pass, message };
      },
    };
  },
  toHaveCss() {
    return {
      compare(actual, expected) {
        const styles = fromPairs(actual.type().rules[0]
         .split('\n')
         .slice(1)
         .slice(0, -1)
         .map((rule) => {
        ...
        const toString = (s) => reduce(s, (aggr, v, k) => `${aggr}${k}: ${v}\n`, '');
        const message = log(pass)`Expected ${actual} to have styles ${toString(expected)} but it had ${toString(styles)}`;
        return { pass, message };
      },
    };
  },
